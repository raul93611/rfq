...
**DB:** Run `sql/bid_pipeline_metrics_migration.sql` — adds `rfq.sources_sought TINYINT(1) DEFAULT 0`. Lost-bid buckets reuse `rfq.comments` (`No Award - Pricing` / `No Award - Technical`) — no schema change. Run `sql/quote_created_at_migration.sql` — adds `rfq.created_at DATETIME NULL DEFAULT CURRENT_TIMESTAMP` (auto on insert; backfilled from `issue_date`), the cohort date for this page. Report-only: SharePoint sync is untouched.

**Status derivation:** `PipelineMetricsRepository::STATUS_CASE` is a SQL `CASE` that mirrors `Rfq::getSheetStatus()` exactly (and in order) — keep the two in sync. 10 buckets: tbd, bid, no_bid, submitted, submitted_ss, award, no_award_pricing, no_award_technical, cancelled, not_submitted. Period filter (`periodClause`) buckets on `rfq.created_at` (the auto quote-creation timestamp; NULL rows drop out) — replaced the hand-typed `issue_date`, whose unparseable values silently dropped rows. Existing rows were backfilled `created_at = issue_date`, so the cohort is unchanged for historical data. Category = `type_of_bid`; priced = `completado = 1`.
