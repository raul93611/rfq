...
**Submitted Date filter + column** — from/to range on `rfq.fecha_submitted` (already-existing nullable `DATE` column, no migration), AND-combined like the other date filters; a row with no `fecha_submitted` never matches once either bound is set. Field sits right after Designated User (double-width `.pt-field-wide`, reuses `.pt-daterange`/`.pt-date` styles ported from the Claude Design handoff — parallel to the `pm-daterange` custom-period picker but scoped to the `pt-` namespace); column sits right after End Date. Counts as one filter (either bound set). Tests: `tests/php/pipeline_table_test.php`, `tests/specs/09-pipeline-metrics.spec.js`.
...
**Re-quote CC payment terms silently didn't reach storage/PDF** — two separate bugs, both since the markup moved from radio buttons to `<select>` dropdowns (`7a9d3c6e`, "50/50 payment term"): (1) `#re_quote_form`'s submit handler recalculated `total_cost` with a leftover `$('input:radio[name=payment_terms]:checked')` selector that no longer matches anything, so it silently clobbered the (correct) interval-computed value with a CC-fee-free one right before every Save — the live on-screen preview was always right, only the persisted/PDF total was wrong. (2) `calcServices()`'s live preview used `1.0299` while `ReQuoteServiceRepository::calc_items_with_CC()` persists with `1.03` (now `ROUND(... CAST(... AS DECIMAL(10,4)), 2)`, mirroring the main quote's `d105b615` tie-rounding fix that was never ported here) — live and saved/PDF services totals silently diverged by a few cents. Both selectors now match `calculateTotals()`'s pattern (`$('[name=payment_terms]')`, no `input:radio`). Tests: `tests/php/re_quote_cc_profit_test.php`, `tests/specs/14-re-quote-cc-payment-terms.spec.js`.
...
**Items table scroll reset on refresh** — `#table_items_container` (`RepositorioItem::escribir_items()`) is its own scrollable pane (`overflow: auto; max-height: 700px`, sticky thead — `css/estilos.css`), not `window` or `.content-wrapper`. `refreshItemsTable()` (`js/quote.js`, the single choke point behind every items-table mutation: add/edit/delete item, subitem, provider) replaces `#items-section-wrapper` — which contains `#table_items_container` — via a full HTML swap, so the container is recreated from scratch and its `scrollTop` resets to 0 on every mutation. Became more noticeable once Add modals started closing on save (above) and dropping users right back onto the table. Fixed by capturing `#table_items_container`'s `scrollTop` before the AJAX call and restoring it on the new element after the swap/reinit. Test: `tests/specs/03-quote-editing-items.spec.js`.
