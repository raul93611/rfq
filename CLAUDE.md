...
**Items table scroll reset on refresh** — `refreshItemsTable()` (`js/quote.js`, the single choke point behind every items-table mutation: add/edit/delete item, subitem, provider) replaced `#items-section-wrapper` via a full HTML swap with no scroll-position handling. Became more noticeable once Add modals started closing on save (above) and dropping users right back onto the table. Fixed by capturing `window.scrollY` before the AJAX call and restoring it after the swap/reinit — `window`, not `.content-wrapper`, is the real scrolling container here; AdminLTE's `layout-fixed` doesn't actually confine it to an inner scrollable pane in this app despite what it sounds like it should do. Test asserts the restore contract via a `window.scrollTo` spy rather than the visible reflow, since a real Chrome run of the literal repro doesn't organically move `window.scrollY` — jQuery's `.html()` sets `innerHTML` in one shot with no intermediate empty state to clamp against. Test: `tests/specs/03-quote-editing-items.spec.js`.
