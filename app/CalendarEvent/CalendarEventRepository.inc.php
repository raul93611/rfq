...
class CalendarEventRepository {
  ...
  public static function saveSharedEvent($connection, $event) {
    if (isset($connection)) {
      try {
        $sql = "
        INSERT INTO calendar_events (id_personnel, name, start, end, color)
        SELECT personnel.id, :name, STR_TO_DATE(:start, '%m/%d/%Y'), STR_TO_DATE(:end, '%m/%d/%Y'), :color
        FROM personnel;
        ";
        $sentence = $connection->prepare($sql);
        $sentence->bindValue(':name', $event->getName(), PDO::PARAM_STR);
