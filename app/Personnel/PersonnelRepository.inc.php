...
class PersonnelRepository{
  ...
  public static function array_to_object($sentence){
    ...
    while ($row = $sentence-> fetch(PDO::FETCH_ASSOC)) {
      $objects[] = new Personnel($row['id'], $row['name']);
    }
  ...
  public static function single_result_to_object($sentence){
    ...
    $object = new Personnel($row['id'], $row['name']);
  ...
  public static function save($connection, $personnel){
    if(isset($connection)){
      try{
        $sql = 'INSERT INTO personnel(name) VALUES(:name)';
  ...
  public static function update($connection, $name, $id){
    if(isset($connection)){
      try{
        $sql = 'UPDATE personnel SET name = :name WHERE id = :id';
        $sentence = $connection-> prepare($sql);
