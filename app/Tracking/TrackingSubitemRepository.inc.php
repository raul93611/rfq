...
class TrackingSubitemRepository {
  ...
  public static function insert_tracking($connection, $tracking) {
    if (isset($connection)) {
      try {
        $sql = 'INSERT INTO trackings_subitems (id_subitem, quantity, carrier, tracking_number, delivery_date, due_date, signed_by, comments) VALUES (:id_subitem, :quantity, :carrier, :tracking_number, STR_TO_DATE(:delivery_date, "%m/%d/%Y"), STR_TO_DATE(:due_date, "%m/%d/%Y"), :signed_by, :comments)';
    ...
    return false; // Return false if connection is not set
  }
