...
class TrackingRepository{
  public static function tracking_list_items($id_quote){
  ...
  public static function tracking_list_item($item, $re_quote_item, $i){
    ...
    <tr>
      <td class="align-middle text-center" rowspan="<?php echo $trackings_quantity; ?>">
        <button type="button" class="add_tracking_button btn btn-warning" name="<?php echo $item-> get_id(); ?>"><i class="fas fa-plus"></i></button>
      </td>
  ...
  if(count($trackings)){
        ...
        <td><?php echo CommentRepository::mysql_date_to_english_format($trackings[0]-> get_delivery_date()); ?></td>
    ...
    for ($j = 1; $j < count($trackings); $j++) {
      ...
      <tr>
        ...
        <td><?php echo CommentRepository::mysql_date_to_english_format($tracking-> get_delivery_date()); ?></td>
  ...
  public static function get_all_trackings_by_id_item($database, $id_item){
    $trackings = [];
    if(isset($database)){
      try{
  ...
  public static function tracking_list_subitem($subitem, $re_quote_subitem){
    ...
    Database::open_connection();
    $trackings_subitems = TrackingRepository::get_all_trackings_by_id_subitem(Database::get_connection(), $subitem-> get_id());
    Database::close_connection();
    ...
    <tr>
      <td class="align-middle text-center" rowspan="<?php echo $trackings_quantity; ?>">
        <button type="button" class="add_tracking_subitem_button btn btn-warning" name="<?php echo $subitem-> get_id(); ?>"><i class="fas fa-plus"></i></button>
      </td>
    ...
    if(count($trackings_subitems)){
          ...
          <td><?php echo CommentRepository::mysql_date_to_english_format($trackings_subitems[0]-> get_delivery_date()); ?></td>
  ...
  public static function insert_tracking($database, $tracking){
    if(isset($database)){
      try{
        ...
        $query = $database-> prepare($sql);
        $query-> bindParam(':id_item', $tracking-> get_id_item(), PDO::PARAM_STR);
  ...
  public static function get_tracking_by_id($database, $id_tracking){
    $tracking = null;
    if(isset($database)){
      try{
        $sql = 'SELECT * FROM trackings WHERE id = :id_tracking';
        $query = $database-> prepare($sql);
        $query-> bindParam(':id_tracking', $id_tracking, PDO::PARAM_STR);
        $query-> execute();
  ...
  public static function update_tracking($database, $quantity, $tracking_number, $delivery_date, $signed_by, $id_tracking){
    if(isset($database)){
      try{
  ...
  public static function delete_tracking($database, $id_tracking){
    if(isset($database)){
      try{
