...
class ReQuoteAuditTrailRepository{
  public static function insert_audit_trail($connection, $audit_trail){
    if(isset($connection)){
      try{
        $sql = 'INSERT INTO re_quote_audit_trails(id_re_quote, username, action_type, id_user, audit_trail, created_date) VALUES(:id_re_quote, :username, :action_type, :id_user, :audit_trail, NOW())';
      ...
      try{
        ...
        if(count($result)){
          foreach ($result as $fila) {
            $audit_trails[] = new ReQuoteAuditTrail($fila['id'], $fila['id_re_quote'], $fila['username'], $fila['action_type'], $fila['id_user'], $fila['audit_trail'], $fila['created_date']);
          }
        }
      }catch(PDOException $ex){
        print 'ERROR:' . $ex->getMessage() . '<br>';
  ...
  public static function create_audit_trail_item_modified($connection, $field_name, $field, $original_field, $id_item, $id_re_quote){
    ...
    $audit_trail = new ReQuoteAuditTrail('', $id_re_quote, $_SESSION['user']-> obtener_nombre_usuario(), 'item_modified', $_SESSION['user']-> obtener_id(), $message, '');
  ...
  public static function create_audit_trail_modified($connection, $field_name, $field, $original_field, $id_re_quote){
    ...
    $audit_trail = new ReQuoteAuditTrail('', $id_re_quote, $_SESSION['user']-> obtener_nombre_usuario(), 'field_modified', $_SESSION['user']-> obtener_id(), $message, '');
    self::insert_audit_trail($connection, $audit_trail);
  }
  ...
  public static function create_audit_trail_item_created($connection, $id_item, $object, $field, $field_name, $id_re_quote){
    ...
    $audit_trail = new ReQuoteAuditTrail('', $id_re_quote, $_SESSION['user']-> obtener_nombre_usuario(), 'item_created', $_SESSION['user']-> obtener_id(), $message, '');
    self::insert_audit_trail($connection, $audit_trail);
  }
  ...
  public static function create_audit_trail_item_deleted($connection, $object, $field, $field_name, $id_re_quote){
    ...
    $audit_trail = new ReQuoteAuditTrail('', $id_re_quote, $_SESSION['user']-> obtener_nombre_usuario(), 'item_deleted', $_SESSION['user']-> obtener_id(), $message, '');
    self::insert_audit_trail($connection, $audit_trail);
  }
