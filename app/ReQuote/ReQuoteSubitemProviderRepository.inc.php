...
class ReQuoteSubitemProviderRepository{
  public static function insert_re_quote_subitem_provider($database, $re_quote_subitem_provider){
    if(isset($database)){
      try{
        ...
        $query-> bindParam(':id_re_quote_subitem', $re_quote_subitem_provider-> get_id_re_quote_subitem(), PDO::PARAM_STR);
        $query-> bindParam(':provider', $re_quote_subitem_provider-> get_provider(), PDO::PARAM_STR);
        $query-> bindParam(':price', $re_quote_subitem_provider-> get_price(), PDO::PARAM_STR);
        $query-> execute();
  ...
  public static function get_re_quote_subitem_providers_by_id_re_quote_subitem($database, $id_re_quote_subitem){
  ...
  public static function get_re_quote_subitem_provider_by_id($database, $id_re_quote_subitem_provider){
    $re_quote_subitem_provider = null;
    if(isset($database)){
      try{
        $sql = 'SELECT * FROM re_quote_subitem_providers WHERE id = :id_re_quote_subitem_provider';
        $query = $database-> prepare($sql);
        $query-> bindParam(':id_re_quote_subitem_provider', $id_re_quote_subitem_provider, PDO::PARAM_STR);
        $query-> execute();
  ...
  public static function update_re_quote_subitem_provider($database, $provider, $price, $id_re_quote_subitem_provider){
    if(isset($database)){
      try{
  ...
  public static function delete_re_quote_subitem_provider($database, $id_re_quote_subitem_provider){
    if(isset($database)){
      try{
