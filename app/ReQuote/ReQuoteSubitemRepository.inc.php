  ...
  public static function insert_re_quote_subitem($connection, $re_quote_subitem){
    if(isset($connection)){
      try{
        ...
        $sentence-> bindValue(':id_re_quote_item', $re_quote_subitem-> get_id_re_quote_item(), PDO::PARAM_STR);
        $sentence-> bindValue(':brand', $re_quote_subitem-> get_brand(), PDO::PARAM_STR);
        $sentence-> bindValue(':brand_project', $re_quote_subitem-> get_brand_project(), PDO::PARAM_STR);
        $sentence-> bindValue(':part_number', $re_quote_subitem-> get_part_number(), PDO::PARAM_STR);
        $sentence-> bindValue(':part_number_project', $re_quote_subitem-> get_part_number_project(), PDO::PARAM_STR);
        $sentence-> bindValue(':description', $re_quote_subitem-> get_description(), PDO::PARAM_STR);
        $sentence-> bindValue(':description_project', $re_quote_subitem-> get_description_project(), PDO::PARAM_STR);
        $sentence-> bindValue(':quantity', $re_quote_subitem-> get_quantity(), PDO::PARAM_STR);
        $sentence-> bindValue(':unit_price', $re_quote_subitem-> get_unit_price(), PDO::PARAM_STR);
        $sentence-> bindValue(':total_price', $re_quote_subitem-> get_total_price(), PDO::PARAM_STR);
        $sentence-> bindValue(':comments', $re_quote_subitem-> get_comments(), PDO::PARAM_STR);
        $sentence-> bindValue(':website', $re_quote_subitem-> get_website(), PDO::PARAM_STR);
        $sentence-> bindValue(':additional', $re_quote_subitem-> get_additional(), PDO::PARAM_STR);
        $sentence-> execute();
  ...
  public static function delete_re_quote_subitem($connection, $id_re_quote_subitem){
    if(isset($connection)){
      try{
        ...
        $sentence1-> bindValue(':id_re_quote_subitem', $id_re_quote_subitem, PDO::PARAM_STR);
        $sentence1-> execute();
        $sql2 = 'DELETE FROM re_quote_subitems WHERE id = :id_re_quote_subitem';
        $sentence2 = $connection-> prepare($sql2);
        $sentence2-> bindValue(':id_re_quote_subitem', $id_re_quote_subitem, PDO::PARAM_STR);
        $sentence2-> execute();
        $connection-> commit();
      }catch(PDOException $ex){
        print 'ERROR:' . $ex->getMessage() . '<br>';
  ...
  public static function insert_calc($connection, $unit_price_subitem, $total_price_subitem, $additional_subitem, $id_re_quote_subitem){
    if(isset($connection)){
      try{
        ...
        $sentence = $connection-> prepare($sql);
        $sentence-> bindValue(':unit_price', $unit_price_subitem, PDO::PARAM_STR);
        $sentence-> bindValue(':total_price', $total_price_subitem, PDO::PARAM_STR);
        $sentence-> bindValue(':additional', $additional_subitem, PDO::PARAM_STR);
        $sentence-> bindValue(':id_re_quote_subitem', $id_re_quote_subitem, PDO::PARAM_STR);
        $sentence-> execute();
      } catch (PDOException $ex) {
        print 'ERROR:' . $ex->getMessage() . '<br>';
