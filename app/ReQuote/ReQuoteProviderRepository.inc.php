...
class ReQuoteProviderRepository{
  public static function insert_re_quote_provider($database, $re_quote_provider){
    if(isset($database)){
      try{
        $sql = 'INSERT INTO re_quote_providers(id_re_quote_item, provider, price) VALUES(:id_re_quote_item, :provider, :price)';
        $query = $database-> prepare($sql);
        $query-> bindParam(':id_re_quote_item', $re_quote_provider-> get_id_re_quote_item(), PDO::PARAM_STR);
        $query-> bindParam(':provider', $re_quote_provider-> get_provider(), PDO::PARAM_STR);
        $query-> bindParam(':price', $re_quote_provider-> get_price(), PDO::PARAM_STR);
        $query-> execute();
  ...
  public static function get_re_quote_providers_by_id_re_quote_item($database, $id_re_quote_item){
    $re_quote_providers = [];
    if(isset($database)){
      try{
  ...
  public static function get_re_quote_provider_by_id($database, $id_re_quote_provider){
    $re_quote_provider = null;
    if(isset($database)){
      try{
        $sql = 'SELECT * FROM re_quote_providers WHERE id = :id_re_quote_provider';
        $query = $database-> prepare($sql);
        $query-> bindParam(':id_re_quote_provider', $id_re_quote_provider, PDO::PARAM_STR);
        $query-> execute();
  ...
  public static function update_re_quote_provider($database, $provider, $price, $id_re_quote_provider){
    if(isset($database)){
      try{
  ...
  public static function delete_re_quote_provider($database, $id_re_quote_provider){
    if(isset($database)){
      try{
