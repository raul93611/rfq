...
class RepositorioRfq {
  ...
  public static function check_fulfillment_and_date($conexion, $total_price_confirmation, $multi_year_project, $id_rfq) {
    $rfq_editado = false;
    if (isset($conexion)) {
      try {
        $sql = 'UPDATE rfq SET fullfillment = 1, fulfillment_date = NOW(), total_price_confirmation = :total_price_confirmation, multi_year_project = :multi_year_project WHERE id = :id_rfq';
        $sentencia = $conexion->prepare($sql);
        $sentencia->bindParam(':id_rfq', $id_rfq, PDO::PARAM_STR);
        $sentencia->bindParam(':total_price_confirmation', $total_price_confirmation, PDO::PARAM_STR);
        $sentencia->bindParam(':multi_year_project', $multi_year_project, PDO::PARAM_STR);
        $sentencia->execute();
        if ($sentencia) {
          $rfq_editado = true;
        }
