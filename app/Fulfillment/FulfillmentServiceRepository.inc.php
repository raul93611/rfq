...
class FulfillmentServiceRepository {
  public static function array_to_object($sentence) {
    ...
    while ($row = $sentence->fetch(PDO::FETCH_ASSOC)) {
      $objects[] = new FulfillmentService($row['id'], $row['id_service'], $row['provider'], $row['quantity'], $row['unit_cost'], $row['other_cost'], $row['real_cost'], $row['payment_term'], $row['reviewed'], $row['created_at'], $row['comments']);
    }
  ...
  public static function single_result_to_object($sentence) {
    ...
    $object = new FulfillmentService($row['id'], $row['id_service'], $row['provider'], $row['quantity'], $row['unit_cost'], $row['other_cost'], $row['real_cost'], $row['payment_term'], $row['reviewed'], $row['created_at'], $row['comments']);
  ...
  public static function insert($connection, $fulfillment_service) {
    if (isset($connection)) {
      try {
        $sql = 'INSERT INTO fulfillment_services(id_service, provider, quantity, unit_cost, other_cost, real_cost, payment_term, created_at, comments) VALUES(:id_service, :provider, :quantity, :unit_cost, :other_cost, :real_cost, :payment_term, NOW(), :comments)';
        $sentence = $connection->prepare($sql);
  ...
  public static function update($connection, $id_fulfillment_service, $provider, $quantity, $unit_cost, $other_cost, $real_cost, $payment_term, $comments) {
    if (isset($connection)) {
      try {
        $sql = 'UPDATE fulfillment_services SET provider = :provider, quantity = :quantity, unit_cost = :unit_cost, other_cost = :other_cost, real_cost = :real_cost, payment_term = :payment_term, comments = :comments WHERE id = :id_fulfillment_service';
        $sentence = $connection->prepare($sql);
