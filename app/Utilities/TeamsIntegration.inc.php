...
class TeamsIntegration {
  ...
  public static function sendDirectMessage($recipient_email, $message) {
    if (empty(TEAMS_DM_ENDPOINT)) return;

    $payload = [
      "recipient" => $recipient_email,
      "message"   => $message
    ];
    $curl = curl_init(TEAMS_DM_ENDPOINT);
    curl_setopt($curl, CURLOPT_POST, true);
    curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($payload));
  ...
  public static function notifyDesignatedUserChanged($id_rfq, $email_code, $channel, $new_user, $changed_by) {
      ...
      "body"    => [
        ...
        [
          ...
          "items" => [
            [
              "type"    => "TextBlock",
              "text"    => "You have been assigned to a quote.",
              "wrap"    => true,
              "spacing" => "Medium"
            ],
            [
              ...
              "facts" => [
                ...
                ["title" => "👤 Assigned by", "value" => $changed_by],
              ]
            ]
          ]
        ]
    ...
    self::sendDirectMessage($new_user->obtener_email(), $message);
  }
