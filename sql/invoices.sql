...
(
    (
        ...
        FROM 
            item i
        JOIN 
            fulfillment_items fi ON i.id = fi.id_item
        WHERE 
            i.id_rfq = 76061
    ...
    (
        ...
        FROM 
            subitems si
        JOIN 
            fulfillment_subitems fsi ON si.id = fsi.id_subitem
        WHERE 
            si.id_item IN (SELECT id FROM item WHERE id_rfq = 76061)
            AND fsi.id_invoice IS NOT NULL
        GROUP BY 
            si.id, fsi.id_invoice
    )
    UNION ALL
    (
        SELECT 
        ...
        FROM 
            services s
        JOIN 
            fulfillment_services fs ON s.id = fs.id_service
        WHERE 
            s.id_rfq = 76061
