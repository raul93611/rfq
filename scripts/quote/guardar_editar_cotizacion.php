...
if (isset($_POST['guardar_cambios_cotizacion'])) {
  ...
  RepositorioRfq::insert_calc(
    ...
    $_POST['id_items'] ?? '', 
    $_POST['id_subitems'] ?? '', 
    $_POST['partes_total_price'] ?? '', 
    $_POST['partes_total_price_subitems'] ?? '', 
    $_POST['unit_prices'] ?? '', 
    $_POST['unit_prices_subitems'] ?? '', 
    $_POST['additional'] ?? '', 
    $_POST['additional_subitems'] ?? ''
  );
  RepositorioRfq::update_variables(
    ...
    htmlspecialchars($_POST['shipping'] ?? ''),  
  ...
  if($cotizacion_recuperada-> obtener_canal() == 'Chemonics'){
    ...
    if (!$cotizacion_recuperada->obtener_completado()) {
      ...
      if(isset($_POST['invoice']) && $_POST['invoice'] == 'si'){
        Conexion::abrir_conexion();
        RepositorioRfq::check_invoice(Conexion::obtener_conexion(), $_POST['id_rfq']);
        RepositorioRfq::set_sales_commission(Conexion::obtener_conexion(), $_POST["invoice_date"],$_POST['sales_commission'], $_POST['sales_commission_comment'], $_POST['id_rfq']);
