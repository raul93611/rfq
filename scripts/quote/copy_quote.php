...
list($cotizacion_insertada, $id_rfq_copia) = RepositorioRfq::insertar_cotizacion(Database::get_connection(), $cotizacion_copia);

$rfq_directory = $_SERVER['DOCUMENT_ROOT'] . '/rfq/documents/' . $id_rfq;
$rfq_copia_directory = $_SERVER['DOCUMENT_ROOT'] . '/rfq/documents/' . $id_rfq_copia;
...
if(count($items)){
  ...
  foreach ($items_copias as $item_copia) {
    RepositorioItem::insertar_item(Database::get_connection(), $item_copia);
  }

  $items_copias = RepositorioItem::obtener_items_por_id_rfq(Database::get_connection(), $id_rfq_copia);
  foreach ($items as $i=> $item) {
    ...
    if(count($subitems)){
      ...
      $subitems_copias = RepositorioSubitem::obtener_subitems_por_id_item(Database::get_connection(), $item_copia-> obtener_id());
      foreach ($subitems as $j=> $subitem) {
        ...
        if(count($providers_subitem)){
          ...
          foreach ($providers_subitem_copias as $provider_subitem_copia) {
            RepositorioProviderSubitem::insertar_provider_subitem(Database::get_connection(), $provider_subitem_copia);
          }
        }
      }
    }
    ...
    if(count($providers)){
      ...
      foreach ($providers_copias as $provider_copia) {
        RepositorioProvider::insertar_provider(Database::get_connection(), $provider_copia);
      }
    }
  }
}
