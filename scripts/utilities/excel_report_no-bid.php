...
function fetchAndFillData($spreadsheet, $type, $quarter, $month, $year) {
  try {
    ...
    ExcelRepository::noBidReport($conexion, $type, $quarter, $month, $year, $spreadsheet->getActiveSheet());
  } catch (Exception $e) {
...
function outputSpreadsheet($spreadsheet) {
  ...
  header('Content-Disposition: attachment;filename="NoBidReport.xlsx"');
