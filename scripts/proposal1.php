...
try{
  ...
  <td></td>
      ...
      for ($i = 0; $i < count($items); $i++) {
        $item_description = '';
          $item = $items[$i];
                if (strlen($item-> obtener_description()) >= $limit) {
                  $indice = $limit;
                  ...
                  while(strlen($item_description) >= $limit){
                    $indice = $limit;
            ...
            for($j = 0; $j < count($subitems); $j++){
              $subitem_description = '';
              $subitem = $subitems[$j];
              if (strlen($subitem-> obtener_description()) >= $limit) {
                $indice = $limit;
                ...
                while(strlen($subitem_description) >= $limit){
                  $indice = $limit;
