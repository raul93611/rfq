$(document).ready(function () {
  if(window.location.href.indexOf('#edit_task_modal') != -1 && window.location.href.indexOf('#id') != -1) {
    $('#edit_task_modal form').load('/rfq/load_task/' + window.location.href.substring(window.location.href.indexOf('#id')+4), function(){
      $('#edit_task_modal').modal();
    });
  }

  $('#tasks_board').load('/rfq/load_tasks_board/');
  $('#my_tasks_board').load('/rfq/load_my_tasks_board/');
  let tasks_done_table = $('#tasks_done_table').DataTable({
    ajax: '/rfq/load_tasks_done_table/'
  });
  ...
  $('#add_task_form').submit(function(){
    $.post('/rfq/save_task', $(this).serialize(), function(res){
  ...
  $('#tasks_board, #my_tasks_board, #tasks_done_table').on('click', '.edit_task_button', function(){
    $('#edit_task_modal form').load('/rfq/load_task/' + $(this).attr('data'), function(){
      $('#edit_task_modal').modal();
    });
    return false;
  });
  ...
  $('#edit_task_form').submit(function(){
    $.post('/rfq/update_task', $(this).serialize(), function(res){
